...
// about ImTextureID in imgui.cpp. (SDL is a cross-platform general purpose library for handling
// windows, inputs, OpenGL/Vulkan graphics context creation, etc.) (GL3W is a helper library to
// access OpenGL functions since there is no standard header to access modern OpenGL functions
...
// up 'RenderDrawListsFn' in the ImGuiIO structure) Note that this implementation is little
...
void ImGui_ImplSdlGL3_NewFrame(SDL_Window* window)
{
    ...
    if (SDL_GetWindowFlags(window) & SDL_WINDOW_MOUSE_FOCUS)
        io.MousePos = ImVec2((float)mx, (float)my);  // Mouse position, in pixels (set to -1,-1 if
                                                     // no mouse / on another screen, etc.)
    else
